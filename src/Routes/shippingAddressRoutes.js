...
router.post('/shipping', addressValidations, validate, authMiddleware, createShippingAddress);

// Obtener todas las direcciones del usuario
router.get('/shipping/user/:userid', authMiddleware, getUserAddresses);

// Obtener la dirección por defecto
router.get('/shipping/default', authMiddleware, getDefaultAddress);

// Obtener una dirección específica
router.get('/shipping/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId')
], validate, authMiddleware, getAddressById);

// Actualizar una dirección
router.put('/shipping/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId'),
  ...addressValidations
...
router.patch('/shipping/:addressId/default', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId')
], validate, authMiddleware, setDefaultAddress);

// Eliminar una dirección
router.delete('/shipping/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId')
