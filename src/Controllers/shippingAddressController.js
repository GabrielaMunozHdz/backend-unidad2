...
const createShippingAddress = async (req, res, next) => {
  try {
    const { name, user, address, city, state, postalCode, country, phone, isDefault, addressType } = req.body;
...
const updateShippingAddress = async (req, res, next) => {
  try {
    ...
    const shippingAddress = await ShippingAddress.findOne({ _id: addressId, user });
